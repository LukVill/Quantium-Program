...
singles/couples, and Mainstream - Older retirees.
...
Mainstream Midage families and Young singles/couples in general buy more chips per customer.
...
chip_t_test <- cust_seg_chip[LIFESTAGE == "YOUNG SINGLES/COUPLES" | LIFESTAGE == "MIDAGE SINGLES/COUPLES",]
...
# with equal variance
t.test(x = ms$TOT_SALES, y = nms$TOT_SALES, alternative = "g", var.equal = T, conf.level = 0.95)
# not equal variance
t.test(x = ms$TOT_SALES, y = nms$TOT_SALES, alternative = "g", var.equal = F, conf.level = 0.95)
...
life_prem_brand_data <- data[(LIFESTAGE == "YOUNG SINGLES/COUPLES" | LIFESTAGE == "MIDAGE SINGLES/COUPLES") & grepl("CHIP",toupper(data$PROD_NAME)),c("LYLTY_CARD_NBR","PROD_NAME","LIFESTAGE","PREMIUM_CUSTOMER","BRAND")]

freq_brand <- table(life_prem_brand_data$BRAND)

ggplot() + geom_bar(aes(x = names(freq_brand), y = freq_brand), stat="identity") + labs(title = "Frequencies of Chip Brand Purchases for Young and Midage Singles/Couples", x = "Brand Name", y = "Frequency")
sum(freq_brand)
...
young_brand <- life_prem_brand_data[LIFESTAGE == "YOUNG SINGLES/COUPLES","BRAND"]
young_trans <- life_prem_brand_data[LIFESTAGE == "YOUNG SINGLES/COUPLES","LYLTY_CARD_NBR"]
...
young_rules <- apriori(young_transactions, parameter = list(supp = 0.0001, conf = 0.8))
# remove empty col
...
# get for midage
mid_brand <- life_prem_brand_data[LIFESTAGE == "MIDAGE SINGLES/COUPLES","BRAND"]
mid_trans <- life_prem_brand_data[LIFESTAGE == "MIDAGE SINGLES/COUPLES","LYLTY_CARD_NBR"]
...
mid_rules <- apriori(mid_transactions,parameter = list(supp = 0.0001, conf = 0.8))
...
life_prem_brand_data_v2 <- data[(LIFESTAGE != "YOUNG SINGLES/COUPLES" & LIFESTAGE != "MIDAGE SINGLES/COUPLES") & grepl("CHIP",toupper(data$PROD_NAME)),c("LYLTY_CARD_NBR","PROD_NAME","LIFESTAGE","PREMIUM_CUSTOMER","BRAND")]
...
# apriori
other_rules <- apriori(other_transactions, parameter = list(supp = 0, conf = 0.8))
...
# do t test if average packsize is higher for mainstream young singles/couples as opposed to others

# get mainstream young singles/couples
ms_young <- data[LIFESTAGE == "YOUNG SINGLES/COUPLES" & PREMIUM_CUSTOMER == "Mainstream",]
# get others
not_ms_young <- data[xor(LIFESTAGE != "YOUNG SINGLES/COUPLES", PREMIUM_CUSTOMER != "Mainstream"),]
...
ms_young_rules <- apriori(ms_young_trans, parameter = list(supp = 0.001, conf = 0.8))
ms_young_rules_df <- inspect(ms_young_rules) %>% select(-2) %>% arrange(desc(support))
...
not_ms_young_rules <- apriori(not_ms_young_trans, parameter = list(supp = 0.001,conf = 0.8))
not_ms_young_rules_df <- inspect(not_ms_young_rules) %>% select(-2) %>% arrange(desc(support))
