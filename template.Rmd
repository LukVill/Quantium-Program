...
top5brands <- data %>% group_by(BRAND) %>% count() %>% ungroup() %>% slice_max(n, n = 8) %>% pull(BRAND)

# filter data to only include top 8 brands

top5brand_data <- data %>% filter(BRAND %in% top5brands & PREMIUM_CUSTOMER == "Mainstream")

# graph bar plot with proportions

ggplot(top5brand_data, aes(x = LIFESTAGE, fill = BRAND)) + geom_bar(position = "fill") + theme(axis.text.x = element_text(angle = 90)) + labs(title = "Top 8 Brands Proportions for each Mainstream Lifestage", x = "Lifestage", y = "Proportion", fill = "Brand Names")
